...
install_options=("Cursor Agent Mode (default)" "Cline Extension" "Claude Desktop")
...
if [[ "$USE_CLINE" == "0" ]]; then
    if [[ "$INSTALL_TYPE" == "0" ]]; then
        ...
        print_success "Created Cursor directory: $(dirname "$MCP_FILE")"
    else
        # Claude Desktop
        mkdir -p "$(dirname "$MCP_FILE")"
        print_success "Created Claude Desktop directory: $(dirname "$MCP_FILE")"
    fi
else
    # Cline Extension
    mkdir -p "$CLINE_DIR"
...
while [ $WAIT_COUNT -lt $MAX_WAIT ]; do
    ...
    if grep -q "BrowserUse logging setup complete\|Successfully ran\|completed successfully" "$TEMP_LOG"; then
...
# Define server configuration with new command
NEW_SERVER='{
  "command": "uvx",
  "args": [
    "--from",
...
echo -e "\n${BOLD}${RED}⚠️  IMPORTANT: You must restart Cursor or the MCP server for changes to take effect! ⚠️${NC}"
echo -e "${RED}To restart the MCP server, you can close and reopen Cursor, or restart it from the Command Palette.${NC}"
